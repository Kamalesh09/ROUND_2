...
class ElasticsearchOperations:
    def __init__(self, host='localhost', port=8989, username='elastic', password='zFAt+e4l7bp33s*Ad-Wq'):
        ...
        self.file_path = r"E:\ELK\Employee Sample Data 1.csv"
...
def main():
    ...
    es_ops = ElasticsearchOperations(
        port=8989,
        username='elastic',
        password='zFAt+e4l7bp33s*Ad-Wq')
    
    
    v_nameCollection = 'Hash_kamalesh'
    v_phoneCollection = 'Hash_4745'
    
    print("\n=== Starting Elasticsearch Operations ===\n")
    
    # Step a
    print("\na) Setting v_nameCollection = 'Hash_kamalesh'...")
    print(f"v_nameCollection = {v_nameCollection}")
    
    # Step b
    print("\nb) Setting v_phoneCollection = 'Hash_4745'...")
    print(f"v_phoneCollection = {v_phoneCollection}")
    
    # Step c
    print("\nc) Creating collection 'Hash_kamalesh'...")
    es_ops.createCollection(v_nameCollection)
    
    # Step d
    print("\nd) Creating collection 'Hash_4745'...")
    es_ops.createCollection(v_phoneCollection)
    
    # Step e
    print("\ne) Getting initial employee count for 'Hash_kamalesh'...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step f
    print("\nf) Indexing data in 'Hash_kamalesh' on column 'Department'...")
    es_ops.indexData(v_nameCollection, 'Department')
    
    # Step g
    print("\ng) Indexing data in 'Hash_4745' on column 'Gender'...")
    es_ops.indexData(v_phoneCollection, 'Gender')
    
    # Step h
    print("\nh) Getting updated employee count for 'Hash_kamalesh'...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step i
    print("\ni) Deleting employee with ID 'E02003' from 'Hash_kamalesh'...")
    es_ops.delEmpById(v_nameCollection, 'E02003')
    
    # Step j
    print("\nj) Getting employee count for 'Hash_kamalesh' after deletion...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step k
    print("\nk) Searching 'Hash_kamalesh' for employees in Department 'IT'...")
    es_ops.searchByColumn(v_nameCollection, 'Department', 'IT')
    
    # Step l
    print("\nl) Searching 'Hash_kamalesh' for employees with Gender 'Male'...")
    es_ops.searchByColumn(v_nameCollection, 'Gender', 'Male')
    
    # Step m
    print("\nm) Searching 'Hash_4745' for employees in Department 'IT'...")
    es_ops.searchByColumn(v_phoneCollection, 'Department', 'IT')
    
    # Step n
    print("\nn) Getting department facets for 'Hash_kamalesh'...")
    es_ops.getDepFacet(v_nameCollection)
    
    # Step o
    print("\no) Getting department facets for 'Hash_4745'...")
